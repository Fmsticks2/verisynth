...
var getCid = async (config, cid, gatewayType, options) => {
  ...
  let data;
  let newUrl = `${config == null ? void 0 : config.pinataGateway}/${gatewayType}/${cid}`;
  const params = new URLSearchParams();
  if (options) {
    if (options.width)
  ...
  try {
    ...
    const contentType = ((_b = request.headers.get("content-type")) == null ? void 0 : _b.split(";")[0]) || null;
    if (contentType == null ? void 0 : contentType.includes("application/json")) {
      data = await request.json();
    } else if (contentType == null ? void 0 : contentType.includes("text/")) {
      data = await request.text();
...
var uploadBase64 = async (config, base64String, network, options) => {
  ...
  if ((_c = options == null ? void 0 : options.metadata) == null ? void 0 : _c.keyvalues) {
  ...
  try {
    ...
    const res = await request.json();
    const resData = res.data;
    if (options == null ? void 0 : options.vectorize) {
...
var uploadFile = async (config, file, network, options) => {
  ...
  if (file.size > 94371840) {
    ...
    const name = ((_a = options == null ? void 0 : options.metadata) == null ? void 0 : _a.name) || file.name || "File from SDK";
    let metadata = `filename ${btoa(name)},filetype ${btoa(file.type)},network ${btoa(network)}`;
    if (options == null ? void 0 : options.groupId) {
      metadata += `,group_id ${btoa(options.groupId)}`;
    }
    if ((_b = options == null ? void 0 : options.metadata) == null ? void 0 : _b.keyvalues) {
      metadata += `,keyvalues ${btoa(JSON.stringify(options.metadata.keyvalues))}`;
    }
    if (options == null ? void 0 : options.streamable) {
      metadata += `,streamable ${btoa("true")}`;
    }
    if (options == null ? void 0 : options.car) {
        ...
        try {
          ...
          if (retryCount > maxRetries) {
            ...
            throw new NetworkError(
              ...
              (uploadReq == null ? void 0 : uploadReq.status) || 0,
              {
                error: errorData,
                code: "HTTP_ERROR",
                metadata: {
                  requestUrl: (uploadReq == null ? void 0 : uploadReq.url) || url,
  ...
  data.append("network", network);
  data.append("name", ((_c = options == null ? void 0 : options.metadata) == null ? void 0 : _c.name) || file.name || "File from SDK");
  if (options == null ? void 0 : options.groupId) {
    data.append("group_id", options.groupId);
  }
  if ((_d = options == null ? void 0 : options.metadata) == null ? void 0 : _d.keyvalues) {
    data.append("keyvalues", JSON.stringify(options.metadata.keyvalues));
  }
  if (options == null ? void 0 : options.streamable) {
...
var uploadCid = async (config, cid, options) => {
  ...
  const requestBody = {
    ...
    keyvalues: (_b = options == null ? void 0 : options.metadata) == null ? void 0 : _b.keyvalues,
    group_id: options == null ? void 0 : options.groupId,
    host_nodes: options == null ? void 0 : options.peerAddresses
  };
